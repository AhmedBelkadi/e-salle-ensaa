...
<head>
    ...
    <title>Erreur métier - E-Salle ENSAA</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
</head>
<body class="bg-gray-50">
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-yellow-50 to-gray-100 p-4">
        <div class="text-center max-w-md">
            <div class="mb-8">
                <div class="inline-flex items-center justify-center w-24 h-24 bg-yellow-100 rounded-full">
                    <i class="fas fa-exclamation-triangle w-12 h-12 text-yellow-600"></i>
                </div>
            </div>
            <h1 class="text-4xl font-bold text-gray-900 mb-4">Erreur métier</h1>
            <p class="text-gray-600 mb-2">
                <c:choose>
                    <c:when test="${not empty errorMessage}">${errorMessage}</c:when>
                    <c:when test="${not empty param.message}">${param.message}</c:when>
                    <c:otherwise>Une erreur métier s'est produite.</c:otherwise>
                </c:choose>
            </p>
