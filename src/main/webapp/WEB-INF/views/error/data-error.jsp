...
<head>
    ...
    <title>Erreur de données - E-Salle ENSAA</title>
        ...
        <div class="text-center max-w-md">
            <div class="mb-8">
                <div class="inline-flex items-center justify-center w-24 h-24 bg-red-100 rounded-full">
                    <i class="fas fa-database w-12 h-12 text-red-600"></i>
                </div>
            </div>
            <h1 class="text-4xl font-bold text-gray-900 mb-4">Erreur de données</h1>
            <p class="text-gray-600 mb-2">
                <c:choose>
                    <c:when test="${not empty errorMessage}">${errorMessage}</c:when>
                    <c:when test="${not empty param.message}">${param.message}</c:when>
                    <c:otherwise>Une erreur d'accès aux données s'est produite.</c:otherwise>
                </c:choose>
            </p>
